...
class OpkgEnvironment(CommonEnvironment):
    """
    Environment class for distributions using opkg as package manager (e.g.
    Wind River Linux).

    The packages are retrieved from the database using the ``opkg info`` tool.
    """
    executable_query = 'opkg'
    executable = 'opkg'
    md5_hash_length = 32
    ...
    def get_package_list(cls, ctx=None):
        ...
        """
        result = []
        command_args = [cls.executable_query, 'info']
    ...
    def get_packageinfo_from_packagefile(cls, file_path, ctx=None):
        ...
        :param ctx: ctx, needed for opkg_include_package_arch
        :return: A PackageInfo()-Object with Package-Version and Package-Name.
        """
        command_args_packagename = [cls.executable, 'info', file_path, '--fields', 'Package']
        command_args_version = [cls.executable, 'info', file_path, '--fields', 'Version']
        command_args_arch = [cls.executable, 'info', file_path, '--fields', 'Architecture']
